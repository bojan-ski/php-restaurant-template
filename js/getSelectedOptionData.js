document.addEventListener('DOMContentLoaded', async () => {
    ...
    const fetchMenuItems = async (option) => {
        try {
            ...
            // add menu items content
            if (option == 'promo' || option == 'doručak' || option == 'americke_palacinke' || option == 'slatke_palacinke' || option == 'brusketi' || option == 'burgeri' || option == 'jela_po_porudžbini' || option == 'salate' || option == 'dezerti') menuItemsCategories(option, data, resultContainer)
...
function menuItemsCategories(option, data, resultContainer) {
    if (option == 'doručak') {
        const p = document.createElement('p');
        p.textContent = 'Doručak se služi do 14h'
    ...
    if (option == 'slatke_palacinke') {
        const p = document.createElement('p');
        p.textContent = 'Palačinke naručite po broju.'
    ...
    for (const [category, menuItems] of Object.entries(groupedMenu)) {
        ...
        categoryContainer.className = `category ${category} text-white mb-5`
        ...
        if (category == 'doručak') {
            ...
            categorySubTitle.textContent = 'Doručak se služi do 14h'
            categorySubTitle.className = 'text-center mb-4';
        ...
        menuItems.forEach(item => {
            ...
            menuItemContainer.className = 'menu-item border-top py-3 mb-2'
            categoryContainer.appendChild(menuItemContainer);

            // Create menu item container
            const menuItemData = document.createElement('div');
            menuItemData.className = 'row justify-content-between';
            menuItemContainer.appendChild(menuItemData);

            // ROW ITEM 1
            const leftDiv = document.createElement('div');
            leftDiv.className = 'col-10';
            ...
            rightDiv.className = 'price col-2 text-end fw-bold fs-5';
            rightDiv.textContent = `${item.price} RSD`;
            menuItemData.appendChild(rightDiv);
        });
    }
}

function menuItemsSimple(option, data, resultContainer) {
    if (option == 'slane_palacinke') {
        const paraOne = document.createElement('p');
        paraOne.textContent = 'Palačinke naručite po broju.'
        paraOne.className = 'mb-3 fs-5 text-white';
        resultContainer.appendChild(paraOne);

        const paraTwo = document.createElement('p');
        paraTwo.textContent = 'POHOVANJE PALAČINKI 90 RSD'
        paraTwo.className = 'mb-3 fs-5 text-white text-uppercase';
        resultContainer.appendChild(paraTwo);
    }

    // Create category container
    const categoryContainer = document.createElement('div');
    categoryContainer.className = `category text-white mb-5`
    resultContainer.appendChild(categoryContainer);

    // Create category title
    const categoryTitle = document.createElement('h3');
    categoryTitle.textContent = option == 'sendvič' ? "DOMAĆA POGAČA" : "PONUDA"
    categoryTitle.className = 'category-title text-uppercase text-center fw-bold mb-3';
    categoryContainer.appendChild(categoryTitle);
    ...
    data.forEach(item => {
        // Create menu item container
        const menuItemData = document.createElement('div');
        menuItemData.className = 'row border-top py-3 justify-content-between';
        ...
        // Create menu title
        const menuItemTitle = document.createElement('h6');
        menuItemTitle.className = 'mb-0 menu-item-title fs-5';
        menuItemTitle.textContent = item.title;
        leftDiv.appendChild(menuItemTitle);

        if (item.with_options) {
            const menuItemOptions = document.createElement('p');
            menuItemOptions.className = 'mt-1 menu-item-options';
            menuItemOptions.textContent = item.with_options;
...
function menuItemsComplex(data, resultContainer) {
    ...
    for (const [category, menuItems] of Object.entries(groupedMenu)) {
        ...
        menuItems.forEach(item => {
            ...
            divOne.className = 'col-10';
            menuItemData.appendChild(divOne);

            // Create menu title
            const menuItemTitle = document.createElement('h5');
            menuItemTitle.className = 'menu-item-title fw-bold';
            menuItemTitle.textContent = item.title;
            ...
            if (item.price) {
                ...
                divTwo.className = 'price col-2 text-end fw-bold fs-5';
                divTwo.textContent = `${item.price} RSD`;
            ...
            if (!item.price) {
                ...
                if (item.icon_one) {
                    ...
                    const iconTextOne = document.createElement('p');
                    iconTextOne.className = 'ms-1 mb-0 text-white';
                    iconTextOne.textContent = item.icon_one
                    divElOne.appendChild(iconTextOne);

                    if (item.icon_two) {
                        const divElTwo = document.createElement('div');
                        divElTwo.className = 'd-flex align-items-center inner-block ms-2';
                        ...
                        const iconTexTwo = document.createElement('p');
                        iconTexTwo.className = 'mb-0 text-white';
                        iconTexTwo.textContent = item.icon_two
